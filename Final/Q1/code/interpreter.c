...
int readCommandEXAM(char *parsedCommand[]){
    ...
    // rewind the filepointer to the begining of the file data
    /*
        NOTE:
        I needed to do a manual rewind to the begining of the data section in this case
        since we needed to read from the begining of the file and we don't have a close function
        in this question. This part will be removed for the rest of the questions.
    */
    fseek(active_file_table[fat[fatIndex].ftIndex], sizeof(partition)+sizeof(fat)+
        sizeof(char)*fat[fatIndex].pointers[0]*partition.block_size, SEEK_SET);
