...
// My function in this file:
int exec(char *val1, char *val2, char *val3){
    if((val2 != NULL) && (strcmp(val1,val2) == 0)){
        printf("Error: Script %s already loaded.\n", val1);
        return 1;
    }
    else if((val2 != NULL) && (val3 != NULL) && (strcmp(val1,val3) == 0)){
        printf("Error: Script %s already loaded.\n", val1);
        return 1;
    }
    else if((val2 != NULL) && (val3 != NULL) && (strcmp(val2,val3) == 0)){
        printf("Error: Script %s already loaded.\n", val2);
        return 1;
    }
    
    if(myinit(val1) == 1) return 1;
    if(val2 != NULL) if(myinit(val2) == 1) return 1;
    if(val2 != NULL && val3 != NULL) if(myinit(val3) == 1) return 1;
    
    if(scheduler() == 1) return 1;
    return 0;
}
